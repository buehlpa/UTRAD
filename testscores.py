...
### this script gets all scores from the testset
...
def main():
    ...
    if args.data_set == 'mvtec':
        ...
        dataset_test=ImageDataset_mvtec(args,DATA_PATH,mode='train',train_paths = test_paths, test_paths = None)
        testdataloader = DataLoader(dataset_test, batch_size=2,shuffle=False,num_workers=8,drop_last=False)

        # load experiment dataset pahts
        #_, _ ,_ = get_dataloader(args)
    ...
    transformer.eval()
    score_map = []
    for i,(filename, batch) in enumerate(testdataloader):
        with torch.no_grad():
    ...
    with open( os.path.join(EXPERIMENT_PATH,'all_testscores.pkl'), 'wb') as file:
