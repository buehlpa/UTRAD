...
def main():
    ...
    for epoch in range(start_epoch, args.epoch_num):
        ...
        for i,(filename, batch) in enumerate(train_dataloader):
            ...
            # flops = FlopCountAnalysis(transformer, outputs)
            # print(f"Total FLOPs: {flops.total()}")

            # Optional: Print a detailed parameter count table
            ...
            batch_time = time.time() - time_start_batch
        ...
        eval_time_end = time.time() - eval_time    
        epoch_time = time.time() - time_start_batch

        print(f"Epoch time: {epoch_time} Eval time: {eval_time_end}")
