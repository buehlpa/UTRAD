...
from utils.refine_paths import refine_paths
...
def main():
    ...
    with open(os.path.join(EXPERIMENT_PATH,'args_feature_refine.log') ,"a") as args_log:
    ...
    SAVE_DIR= os.path.join(EXPERIMENT_PATH, args.model_result_dir, 'checkpoint_feature_refine.pth')
    ...
    if args.data_set == 'mvtec':
        ...
        # allpaths
        dataset_clean_train=ImageDataset_mvtec(args,DATA_PATH,mode='train',train_paths = experiment_paths['lof_refined_paths'], test_paths = None)
        ...
        print(args.data_category)
        args.data_category =f'{args.data_category}'
        print(args.data_category)
        ...
        print(DATA_PATH)
        # allpaths
        dataset_clean_train=ImageDataset_beantec(args,DATA_PATH,mode='train',train_paths = experiment_paths['lof_refined_paths'], test_paths = None)
        clean_traindataloader = DataLoader(dataset_clean_train, batch_size=2,shuffle=False,num_workers=8,drop_last=False)
        ...
        # allpaths
        dataset_clean_train=ImageDataset_visa(args,DATA_PATH,mode='train',train_paths = experiment_paths['lof_refined_paths'], test_paths = None)
        clean_traindataloader = DataLoader(dataset_clean_train, batch_size=2,shuffle=False,num_workers=8,drop_last=False)
    ...
    for epoch in range(start_epoch, args.epoch_num):
        ...
        with open(os.path.join(EXPERIMENT_PATH,'args_feature_refine.log') ,"a") as train_log:
    ...
    with open( os.path.join(EXPERIMENT_PATH,'all_testscores_refined.pkl'), 'wb') as file:
